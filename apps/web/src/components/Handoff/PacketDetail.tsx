...
export function PacketDetail({ packet, transport }: Props) {
  ...
  return (
    <section className="handoff-detail-card">
      <header className="handoff-detail-header">
        <strong className="handoff-detail-title">{packet.title}</strong>
        <StatusPill status={packet.status} />
      </header>
      {packet.summary && (
        <div className="handoff-meta">{packet.summary}</div>
      )}
      ...
      {packet.tasks.length > 0 && (
        <div className="handoff-section" style={{ marginTop: 10 }}>
          <h4 className="handoff-section-title">Tasks ({packet.tasks.length})</h4>
      ...
      <div className="handoff-section" style={{ marginTop: 10 }}>
        <h4 className="handoff-section-title">Approval</h4>
        <div className="handoff-meta">
      ...
      <div className="handoff-section" style={{ marginTop: 10 }}>
        <h4 className="handoff-section-title">
          Signers ({packet.signers.length}/{packet.required_signers})
        </h4>
        <ul className="soft-list">
          {packet.signers.map((s) => (
            <li key={s.name} className="soft-list-item" style={{ fontSize: 12 }}>
      ...
      <div className="handoff-section" style={{ marginTop: 10 }}>
        <h4 className="handoff-section-title">State history</h4>
        <ul className="soft-list">
          {packet.state_history.map((entry, index) => (
            <li key={`${entry.state}-${entry.at}-${index}`} className="soft-list-item" style={{ fontSize: 12 }}>
      ...
      {packet.status === 'pending_approval' && (
        <section className="handoff-action-panel">
          <h4 className="handoff-section-title">Approve</h4>
          ...
          {err && <div className="handoff-note-error">{err}</div>}
        </section>
      )}
      {packet.status === 'approved' && (
        <div className="handoff-action-panel">
      ...
      <div className="handoff-section" style={{ marginTop: 10 }}>
        <h4 className="handoff-section-title">Execution</h4>
        <div className="handoff-meta">
        ...
        {executionProgressEntries.length > 0 && (
          <div style={{ marginTop: 10 }}>
            <h4 className="handoff-section-title">Task progress</h4>
            <ul className="soft-list">
              {executionProgressEntries.map((progress) => (
                <li key={progress.task_id} className="soft-list-item" style={{ fontSize: 12 }}>
        ...
        {packet.status === 'completed' && (
          <div className="handoff-note-ok">
            Execution completed. Reassessment can run next.
          </div>
        )}
        {packet.status === 'failed' && (
          <div className="handoff-note-error">
