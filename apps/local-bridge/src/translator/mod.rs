...
pub async fn dispatch_command(
    ...
    match cmd.cmd_type.as_str() {
        ...
        "gate.sync_mutation_audit" => {
            crate::gate::handle_sync_mutation_audit(&cmd, &state, principal.clone()).await
        }
        ...
        "release.deploy" => crate::release::handle_deploy(&cmd, &state).await,
        "release.publish" => crate::release::handle_publish(&cmd, &state).await,
        "session.create" => {
            let profile_id = cmd
                .payload
                .get("profile_id")
